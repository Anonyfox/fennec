(* The "say hello" form model — ONE Sift codec shared by BOTH form stories: the server-rendered handler
   (web/handlers/hello.mlx, where [Form.read] coerces + validates the POST and [Form.input_attrs]
   stamps the <input>'s HTML5 constraints) AND the browser SPA component (web/components/hello_form.mlx,
