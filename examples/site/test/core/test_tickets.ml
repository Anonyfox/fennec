(* The non-web core, end to end in the example: the ticket domain (collections/ + workflows/) over the
   ambient in-memory backend. The workflows are called like ORDINARY functions. Proves open_ticket writes
   two collections atomically (and a raise rolls BOTH back), close is a guarded transition, and the
   @after effect rides along. A standalone Eio exe over MONGO_URL=:memory:, run via a runtest rule. *)
...
let tickets () = List.length (Pulse.all Ticket.collection)
let events () = List.length (Pulse.all Ticket_event.collection)
...
let () =
  ...
  check "close persisted the new state" (match Pulse.get Ticket.collection t.Ticket.id with Some x -> x.Ticket.status = "closed" | None -> false);
  ...
  check "closing a closed ticket is vetoed (state intact)"
    (match Pulse.get Ticket.collection t.Ticket.id with Some x -> x.Ticket.status = "closed" | None -> false);
