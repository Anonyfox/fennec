(* Error-panel system test (ported from errors.sh). Provokes a build error through the REAL
   `fennec dev` and asserts the terminal does the right thing. Contained (the whole process group
   is reaped on teardown — no orphans between scenarios).
...
let%system "error panel counts correctly + carries a message, and a revert-to-identical fix clears it" = fun sb ->
  let layout = Filename.concat (S.app_dir ()) "frontend/apps/web/layout.mlx" in
  let dev = S.dev sb in
  ...
  (* 1) provoke a syntax error (remove a delimiter), then 2) fix it by reverting to byte-identical
     output. with_edit injects the error and restores the file (the revert-to-identical case), so
     the panel-clears check runs AFTER with_edit's body. *)
