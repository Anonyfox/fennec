(* Host-routing + port-override system test (ported from domains.sh). Typed, deterministic
   ...
   Guards two properties:
     1. FIDELITY: the dev GATEWAY (:4000) routes by Host EXACTLY as prod will — a specific host
        (admin.localhost) wins, the "*" endpoint (main) is the default, and an unknown host still
...
let%system "host-routing fidelity on the gateway (:4000): specific host -> admin, default + unknown -> main" = fun sb ->
  let dev = S.dev sb in
  S.wait_ready dev ~port:4000 ();

  (* a plain visit (Host: localhost) routes to the main "*" default *)
  S.check "gateway plain visit routes to the main '*' default"
  ...
  (* the main app's own route is present on the gateway *)
  S.check "main app's own route present on the gateway"
    (contains (S.request 4000 "/api/health").S.body {|"app":"main"|});
