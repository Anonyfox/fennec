(* Host-routing + port-override system test (dev-only), ported from domains.sh. Typed,
   deterministic (condition waits, never sleep-and-hope), and contained (the System layer reaps
   the whole process group on teardown — no orphans between scenarios).
...
let _ = root  (* kept for parity with the other suites; not needed here *)

let () = S.main @@ fun () ->

  S.test "host-routing fidelity on the gateway (:4000): specific host -> admin, default + unknown -> web" (fun sb ->
  ...
  S.test "--port override — the whole block shifts to a custom base" (fun sb ->
    let dev = S.spawn sb ~cwd:app_dir [ fennec; "dev"; "--port"; "9000" ] in
