...
**This is not your code.** It exists so the build can compile the *same* `web/` twice — once for
...
Everything here is driven by globs over `web/`, so **adding (or nesting into subfolders) a page,
component, handler, or whole app needs zero changes in here** (or anywhere). dune:

- compiles the `-data-client` / `-conn-client` *mirrors* of your `web/` (`components/`,
  ...
  `dynamic_include`. Nest a file under `web/<cat>/` and its mirror appears automatically.
...
You only ever touch `web/`. Leave this folder alone — and `_build/`, `.fennec/`, and `dist/` too.
