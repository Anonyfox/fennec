...
#
#   A) a LEFTOVER of OUR OWN server holding :4000  -> fennec dev SIGKILLs it and binds (transparent);
#   B) a FOREIGN process holding :4000             -> fennec dev does NOT kill it, and prints a
...
trap cleanup EXIT
held() { lsof -nP -iTCP:4000 -sTCP:LISTEN >/dev/null 2>&1; }
...
i=0; while ! held; do i=$((i+1)); [ $i -gt 20 ] && fail "leftover server never bound :4000"; sleep 0.5; done
...
logged /tmp/reclaim_a.log "ready" || fail "fennec dev never bound the port (didn't reclaim the leftover)"
holder=$(lsof -nP -iTCP:4000 -sTCP:LISTEN -t 2>/dev/null | head -1)
[ -n "$holder" ] && [ "$holder" != "$LEFT" ] || fail "the leftover ($LEFT) still holds :4000 — not reclaimed"
...
# lets fennec's 127.0.0.1:4000 coexist with a 0.0.0.0:4000 holder and there's no conflict
python3 -m http.server 4000 --bind 127.0.0.1 >/dev/null 2>&1 & FOREIGN=$!
i=0; while ! held; do i=$((i+1)); [ $i -gt 20 ] && fail "foreign holder never bound :4000"; sleep 0.5; done
