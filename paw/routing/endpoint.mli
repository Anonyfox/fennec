(** An endpoint: an app's identity (name + host patterns) and its two-phase paw pipeline.
    ...
    {[
      ...
      |> Endpoint.pipe_matched [ Basic_auth.make ~username ~password () ]
    ]} *)
...
val form : string -> Pipeline.t -> t -> t
...
val use_matched : Pipeline.t -> t -> t
...
val pipe_matched : Pipeline.t list -> t -> t
