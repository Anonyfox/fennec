(** Sessions (Dream-grade). A small [string -> string] map per request for login state,
    ...
    logged-in user". Use {!Conn.set_cookie} / {!Conn.delete_cookie} for a
    single unsessioned response cookie.

    {[
      Endpoint.pipe [ Session.make ~secret () ]

      (* downstream: record + read the logged-in user *)
      let conn = Session.set conn "user_id" "42"
      let who = Session.get conn "user_id"
    ]} *)
...
(** The request scheme as the client sees it, honouring an upstream [X-Forwarded-Proto] (behind a
    TLS-terminating proxy {!Conn.scheme} is the inner ["http"]). This is what the [Secure]
    cookie default keys on; exposed so other cookie-issuing code (e.g. the login cookie) matches it. *)
val forwarded_scheme : Conn.t -> string
...
val get : Conn.t -> string -> string option

(** The session map (reserved ["_"]-prefixed keys hidden). *)
val get_all : Conn.t -> (string * string) list
...
val set : Conn.t -> string -> string -> Conn.t

(** Remove one key. *)
val delete : Conn.t -> string -> Conn.t

(** Empty the session. *)
val clear : Conn.t -> Conn.t
