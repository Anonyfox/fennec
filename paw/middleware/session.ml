...
let make ~(secret : string) ?(cookie = "_fennec_session") ?(path = "/") ?(lifetime = 86400.)
    ?(same_site = Cookie.Lax) ?(http_only = true) ?secure ?store () : Pipeline.t =
  ...
  if String.length secret < 16 then
    invalid_arg
      (Printf.sprintf "Fennec.Session.make: ~secret must be at least 16 bytes (got %d) — use a long random string"
...
let set_cookie_ c =
  match Headers.get_all (finalize_ c).H.headers "set-cookie" with s :: _ -> s | [] -> ""
...
let%test "unchanged session emits no Set-Cookie" =
  ...
  Headers.get_all r3.H.headers "set-cookie" = []
