(** HTTP Basic auth — answers 401 with a [WWW-Authenticate] challenge unless the request's
    ...
    For protected routes, attach this paw with {!Endpoint.use_matched} or
    {!Endpoint.pipe_matched} after defining the routes. That guards real matches
    ...
    {[
      ...
      |> Endpoint.pipe_matched
           [ Basic_auth.make ~username:"admin" ~password:"admin" ~realm:"Admin" () ]
    ]} *)

(** Build the basic-auth paw guarding everything downstream. [realm] (default ["Restricted"])
    is shown in the browser's auth prompt. *)
val make : username:string -> password:string -> ?realm:string -> unit -> Pipeline.t
