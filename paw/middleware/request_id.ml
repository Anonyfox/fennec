...
let make ?(header = "x-request-id") () : Pipeline.t =
 fun c ->
  let id =
...
let req_ ?(headers = []) path = Http.make_request ~meth:Http.GET ~path ~headers ()
...
let%test_unit "sets the response header" =
  ...
  let has = match Conn.resp (Conn.text c "x") with
    | Some r -> Headers.mem r.Http.headers "x-request-id"
    | None -> false in
