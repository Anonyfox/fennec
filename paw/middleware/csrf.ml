...
let secret_key : string Assigns.key = Assigns.key "fennec.csrf.secret"
...
let require_session (c : Conn.t) : unit =
  if not (Session.active c) then
    failwith "Fennec.Csrf: no active session — add Session.make earlier in the pipeline"
...
let token ?secret ?(valid_for = 3600.) (c : Conn.t) : string =
  ...
  let secret =
    ...
    | None -> (
        ...
        | Some s -> s
        | None -> failwith "Fennec.Csrf: no secret on the conn — add Csrf.make to the pipeline")
  in
...
let make ~(secret : string) ?(field = "_csrf_token") ?(header = "x-csrf-token")
    ?(safe = [ "GET"; "HEAD"; "OPTIONS" ]) () : Pipeline.t =
  if String.length secret < 16 then
    invalid_arg
      (Printf.sprintf "Fennec.Csrf.make: ~secret must be at least 16 bytes (got %d) — use a long random string"
