...
let is_html_response (r : Http.response) : bool =
  match Http_semantics.header r.Http.headers "content-type" with
...
let paw (t : t) : Pipeline.t =
 fun c ->
  if Conn.path c = Dev.endpoint then
    Conn.upgrade c (fun (ch : Ws_channel.t) ->
        let unregister = register t ch.Ws_channel.send in
        ch.Ws_channel.on_close <- unregister)
  else
        ...
        if is_html_response r then
          ...
          let headers =
            ...
            :: List.filter
                 ...
                 r.Http.headers
          in
          { r with Http.body = Dev.inject_html r.Http.body; headers }
        else r)
