(* The storage backend the reactive surface needs — CRUD over BSON documents (with query options) plus
   field-level live deltas — and the runtime selection of a concrete one from MONGO_URL. This is the
   single seam every layer above talks to, and it lives in the mongo package so all the engine
   machinery (minimongo / embedded Burrow / native driver) is owned here, behind one signature.

   The in-memory minimongo backend ({!Mini}) is here; the native driver, the embedded Burrow engine, the
   runtime selector ({!Dynamic}), and the mongosh wire endpoint are added alongside it. *)
