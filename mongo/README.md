...
- **Aggregation** — `$match`/`$project`/`$addFields`/`$unset`/`$sort`/`$limit`/`$skip`/`$count`/`$unwind`/`$group`/`$sortByCount`/`$sample`/`$replaceRoot`/`$lookup`/`$unionWith`/`$facet`/`$bucket`, the full `Expr` language (field paths, `$$ROOT`, arithmetic/comparison/boolean/conditional/string/array/type), and the accumulators.
