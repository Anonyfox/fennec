...
5. **DDP on fennec's server** — `Paw.Websocket.make "/websocket"` + `/sockjs` shim. ✓ **DONE** —
   ...
   method-error code, sockjs framing) AND over a **real WebSocket**: its live twin
   `fennec/pulse/server/test/test_realtime` is a self-contained `(test)` (CI-run via `dune runtest`)
   — a minimal RFC 6455 server (fennec's
