(* Typed form INPUT over the Sift model — the input half of a handler. The stringly request a form
   (or query string) submits is COERCED into the shape's BSON (driven by [Sift.view]) and run through
...
let rec coerce (view : Sift.view) (s : string) : Bson.t option =
  match view with
  | Sift.V_string | Sift.V_id | Sift.V_bson -> Some (Bson.str s)
  | Sift.V_int -> Option.map Bson.int (int_of_string_opt (String.trim s))
  | Sift.V_float -> Option.map Bson.float (float_of_string_opt (String.trim s))
  | Sift.V_bool -> Some (Bson.bool (truthy s))
  | Sift.V_date -> Option.map (fun ms -> Bson.Date ms) (Int64.of_string_opt (String.trim s))
  | Sift.V_option inner -> if String.trim s = "" then Some Bson.Null else coerce inner s
  | Sift.V_check (_, inner) -> coerce inner s
  | Sift.V_unit -> Some Bson.Null
  | Sift.V_list _ | Sift.V_map _ | Sift.V_obj _ | Sift.V_variant _ -> None

let coerce_msg (view : Sift.view) =
  match strip view with Sift.V_int | Sift.V_float -> "must be a number" | Sift.V_date -> "must be a date" | _ -> "invalid value"

(* the structural kind with check AND option wrappers peeled (for dispatch) *)
let rec kind (v : Sift.view) : Sift.view = match v with Sift.V_check (_, i) | Sift.V_option i -> kind i | i -> i
...
let parse_assoc (c : 'a Sift.t) (data : (string * string) list) : ('a, Sift.error list) result =
  match Sift.view c with
  | Sift.V_obj top_fields ->
      let coerce_errs = ref [] and failed = ref [] in
      let add_fail path msg =
        failed := path :: !failed;
        coerce_errs := Sift.err ~code:"coerce" path msg :: !coerce_errs
      in
      ...
      let rec build fields path key : (string * Bson.t) list =
        List.filter_map
          (fun (name, _required, shape) ->
            ...
            | Sift.V_bool -> Some (name, Bson.bool (match values_of k with v :: _ -> truthy v | [] -> false))
            | Sift.V_list inner ->
                let elems =
                  List.filter_map
      ...
      (match Sift.decode c (Bson.Document bson_fields) with
      | Ok v -> if !coerce_errs = [] then Ok v else Error (List.rev !coerce_errs)
      | Error es ->
          let es' = List.filter (fun (e : Sift.error) -> not (List.mem e.path !failed)) es in
          Error (List.rev !coerce_errs @ es'))
  | _ -> Error [ Sift.err [] "form codec must be a record" ]
...
let read ?(from = Body) ?(inject = []) ?(ignore = []) (c : 'a Sift.t) (conn : Conn.t) : ('a, Sift.error list) result =
...
let field_errors (f : _ Sift.field) (errs : Sift.error list) : string list =
  let n = Sift.field_name f in
  List.filter_map (fun (e : Sift.error) -> match e.path with x :: _ when x = n -> Some e.msg | _ -> None) errs
...
let summary (errs : Sift.error list) : error_summary =
  let form_errors = List.filter_map (fun (e : Sift.error) -> match e.path with [] -> Some e.msg | _ -> None) errs in
  let pairs = List.filter_map (fun (e : Sift.error) -> match e.path with f :: _ -> Some (f, e.msg) | [] -> None) errs in
...
type ctx = { conn : Conn.t; flash : string option; errors : Sift.error list }
...
let error (c : ctx) (name : string) : Fur.vnode =
  match
    List.filter_map (fun (e : Sift.error) -> match e.path with x :: _ when x = name -> Some e.msg | _ -> None) c.errors
  with
  | [] -> Fur.frag []
...
type outcome =
  | Again of Sift.error list  (** re-render [view] with these field errors, 422 — codec-invalid input auto-does this *)
...
let again (fields : (string * string) list) : outcome = Again (List.map (fun (f, m) -> Sift.err [ f ] m) fields)
...
let f_age = Sift.(req "age" (min_i 18 int))
...
let%test "summary groups errors by field and separates form-level ones" =
  let s = summary [ Sift.err [] "form bad"; Sift.err [ "email" ] "is required"; Sift.err [ "email" ] "is invalid"; Sift.err [ "age" ] "too small" ] in
...
let addr_codec = Sift.(seal (record (fun city zip -> { city; zip }) |> field (req "city" (non_empty string)) (fun a -> a.city) |> field (req "zip" string) (fun a -> a.zip)))
let person_codec = Sift.(seal (record (fun who home -> { who; home }) |> field (req "who" (non_empty string)) (fun p -> p.who) |> field (req "home" addr_codec) (fun p -> p.home)))
...
let%test "read: dotted keys populate an embedded record; a missing nested field errors at its path" =
  ...
  && match parse_assoc person_codec [ ("who", "Ada"); ("home.zip", "NW1") ] with
     | Error errs -> List.exists (fun (e : Sift.error) -> e.path = [ "home"; "city" ]) errs
     | Ok _ -> false
...
let%test "flash renders a banner when set, empty when not; error keys by field name" =
  ...
  let fe = ctx ~conn:c0 ~flash:None ~errors:[ Sift.err [ "name" ] "must not be empty" ] in
...
let%test "again/redirect/page carry their payload at the right path" =
  (match again [ ("name", "taken") ] with Again [ e ] -> e.Sift.path = [ "name" ] && e.Sift.msg = "taken" | _ -> false)
