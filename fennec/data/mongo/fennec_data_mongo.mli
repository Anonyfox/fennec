(** Native MongoDB backend — a {!Fennec_data.Backend.S} over the libmongoc driver
    ({!Fennec_mongo_driver}), so the whole reactive / DDP / realtime stack runs over a real mongod
    with no other change (it is the same seam {!Fennec_data.Backend.Mini} implements in memory):
    [Reactive.Make (Fennec_data_mongo)].
