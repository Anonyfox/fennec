(** The stub's write surface — {!Fennec_pulse_method.Method.sim_writes} over the client merge store.
...
(** [writes box ~sim ~seed] registers [sim] (via {!Merge_store.begin_sim}) and returns the write
    surface bound to it. *)
val writes : Merge_store.t -> sim:string -> seed:string -> Fennec_pulse_method.Method.sim_writes
