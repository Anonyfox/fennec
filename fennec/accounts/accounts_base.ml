(* accounts_base.ml — the Accounts engine, a thin glue re-exporting the cohesive engine modules
   in dependency order so the public surface (via accounts.ml's [include Accounts_base] ->
   accounts.mli) is unchanged. Bodies live in Accounts_{types,secrets,identity_bridge,runtime,
   request,...}; the remaining lifecycle/login/http bodies are folded in below until carved. *)
