(* accounts_base.ml — the Accounts engine, now a thin glue that re-exports the cohesive engine
   modules in dependency order so its public surface (consumed by accounts.ml via [include
   Accounts_base], which in turn satisfies accounts.mli) is unchanged. The bodies live in:
     Accounts_types            — the data model + aliases + Assigns keys + trivial helpers
     Accounts_secrets          — CSPRNG/hashing/constant-eq, the session codec, the MFA seal
     Accounts_identity_bridge  — external_identity + per-provider identity glue + services
   (more engine layers are folded in below as they are carved out.) *)
