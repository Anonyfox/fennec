...
(** The typed-BSON shape language, re-exported for building a {!profile_codec} — describe the optional
    typed [user.profile] as a ['p Sift.t] (by hand or via [[@@deriving model]]). *)
module Sift = Sift
...
(** {1 Optional typed [user.profile] (Sift)}

    [user.profile] stays an opaque [Bson.t option] the framework never interprets. These are a thin,
    additive convenience for apps that want a {e typed} profile: describe the profile shape once as a
    ['p Sift.t] codec (by hand, or generated by [[@@deriving model]]), wrap it with {!profile_codec},
    and read/write [user.profile] through it. No storage change — a typed profile encodes to the same
    BSON that lands in the slot; reads decode it back. Fail-safe: a missing OR foreign-shaped profile
    reads as [None], never a raise or a half-decoded value. *)

(** A typed-profile handle: a ['p Sift.t] codec for the profile shape. Holds no Accounts state, so one
    handle works for any instance / user. *)
type 'p profile_codec

(** Wrap a Sift codec as a {!profile_codec}. *)
val profile_codec : 'p Sift.t -> 'p profile_codec

(** Decode [user.profile] through the codec, fail-safe: [None] for an absent profile {e or} a profile
    that does not match this codec's shape. Use {!typed_profile_result} when "absent" must be told apart
    from "present but wrong shape". *)
val typed_profile : 'p profile_codec -> user -> 'p option

(** Like {!typed_profile} but surfacing the decode outcome: [Ok None] (no profile), [Ok (Some p)] (a
    profile that matched), or [Error es] (a profile present but not of this shape, with the Sift
    violations). *)
val typed_profile_result : 'p profile_codec -> user -> ('p option, Sift.error list) result

(** Encode a typed value to the BSON profile slot and persist it through the same store path as
    {!set_profile}. [Sift.to_bson] is total, so this only fails for the usual store reasons. *)
val set_typed_profile : t -> 'p profile_codec -> user_id -> 'p -> (user, error) result

(** Clear the profile (the [None] write), for symmetry with {!set_typed_profile}. *)
val clear_profile : t -> user_id -> (user, error) result
