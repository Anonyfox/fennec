...
val token : ?secret:string -> ?valid_for:float -> Paw.Conn.t -> string
...
val token_opt : ?valid_for:float -> Paw.Conn.t -> string option

(** Validate a submitted token against the app [secret] and the session secret. *)
val verify : secret:string -> Paw.Conn.t -> string -> outcome
...
val make :
  secret:string -> ?field:string -> ?header:string -> ?safe:string list -> unit -> Paw.t
