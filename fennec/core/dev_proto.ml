...
(* the dev-URL report: "[fennec:urls] http://localhost:8200 http://localhost:8201" *)
let urls_line (urls : string list) : string = urls_prefix ^ " " ^ String.concat " " urls

let parse_urls_line (line : string) : string list option =
  if not (starts_with line urls_prefix) then None
  else
    let lp = String.length urls_prefix in
    Some (String.sub line lp (String.length line - lp) |> String.split_on_char ' ' |> List.filter (fun s -> s <> ""))
