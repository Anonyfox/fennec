(* The js_of_ocaml BACKEND for the core reconciler, plus the hydrate/render entry. The
   diff algorithm itself lives in Fur.Reconcile (core, platform-agnostic + unit-tested
   with a fake backend); here we only map its operations onto the real DOM. *)
open Js_of_ocaml

module Jsoo : Fur.BACKEND with type node = Dom.node Js.t = struct
...
let hydrate_root (container : Dom_html.element Js.t) (render : unit -> Fur.vnode) =
