(* Client-side head reconciler. Reads Fur.Head.sources (reactive) and patches
   document.head to match the resolved tag set, keyed by data-fh.

   Hydration-safe: it SEEDS its key->element map from the SSR'd [data-fh] tags
...
let start () =
  ...
  let existing = head##querySelectorAll (Js.string "[data-fh]") in
  for i = 0 to existing##.length - 1 do
    ...
    let k = Js.to_string (Js.Opt.get (el##getAttribute (Js.string "data-fh")) (fun () -> Js.string "")) in
  ...
  let mk key (t : Head.tag) =
    ...
    in
    el##setAttribute (Js.string "data-fh") (Js.string key);
    el
  in
