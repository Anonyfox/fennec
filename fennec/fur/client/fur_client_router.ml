(* Client navigation for Fur.Router: sets the initial relative path from the URL,
...
let install (t : Fur.Router.t) =
  (* initial relative path BEFORE hydration, so the first client render == SSR *)
  Fur.Router.set_path t (pathname ());
  (* navigation now lives in core: Fur.navigate = Platform.push_state + sync_path +
     flush_mounts; Fur.sync_path = path + flush (for popstate, already navigated). *)
  ignore (Dom_html.addEventListener Dom_html.window Dom_html.Event.popstate
            (Dom_html.handler (fun _ -> Fur.sync_path (pathname ()); Js._true)) Js._false);
