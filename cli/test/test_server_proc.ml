...
let () =
  ...
  check "a urls report -> Urls" (S.classify_line "[fennec:urls] web=http://localhost:8200 admin=http://localhost:8201" = S.Urls [ ("web", "http://localhost:8200"); ("admin", "http://localhost:8201") ]);
  ...
  check "leading/trailing space on a urls line still parses" (S.classify_line "  [fennec:urls] web=http://x  " = S.Urls [ ("web", "http://x") ]);
