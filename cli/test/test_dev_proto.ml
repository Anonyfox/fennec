...
let () =
  ...
  check "urls round-trips" (P.parse_urls_line (P.urls_line [ ("web", "http://localhost:8200"); ("admin", "http://localhost:8201") ]) = Some [ ("web", "http://localhost:8200"); ("admin", "http://localhost:8201") ]);
  check "urls round-trips (single)" (P.parse_urls_line (P.urls_line [ ("web", "http://localhost:8200") ]) = Some [ ("web", "http://localhost:8200") ]);
  check "urls round-trips (empty)" (P.parse_urls_line (P.urls_line []) = Some []);
  check "a url with '=' in it (query) splits on the FIRST '='" (P.parse_urls_line (P.urls_line [ ("web", "http://x/?a=1") ]) = Some [ ("web", "http://x/?a=1") ]);
  check "urls line carries the prefix" (P.starts_with (P.urls_line [ ("web", "x") ]) P.urls_prefix);
  ...
  check "parse_port rejects a urls line" (P.parse_port_busy (P.urls_line [ ("web", "http://x") ]) = None);
  ...
  check "a urls line is NOT chatter" (not (P.starts_with (P.urls_line [ ("web", "x") ]) P.chatter_prefix));
