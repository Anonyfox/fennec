(* Doc-coverage for [fennec docs]: parse an interface (or implementation) and report exports that
   lack a [(** ... *)] doc comment. OCaml has no missing-docs lint; this is fennec's. Because odoc
   renders the curated [.mli], an export is documented-in-[.mli] (renders), documented-only-in-[.ml]
   (won't render — "port" it), or undocumented. *)
...
(** Insert a doc comment before each given 1-based line of a source string (the [--port] core).
    One pass, so line numbers never shift; a doc that would close the comment early is skipped.
    Pure. *)
val port_source : string -> (int * string) list -> string
...
(** Run the check over [paths] (files or directories; default: the current project).
    ...
    - [port] → don't report; instead copy each ".ml-only" doc into its [.mli] (idempotent; the
      [.mli] wins on conflict; the [.ml] is never modified). Returns the process exit code. *)
val run : paths:string list -> strict:bool -> private_:bool -> port:bool -> int
