...
let trust_hash_identity ~command =
  let identity =
    "{\"event_name\":\"post_tool_use\",\"hooks\":[{\"async\":false,\"command\":" ^ Harness.json_escape command
...
let trust_table ~hooks_path = "[hooks.state.\"" ^ hooks_path ^ ":post_tool_use:0:0\"]"
...
let install_trust ~command =
  let path = config_path () in
  let table = trust_table ~hooks_path:(hooks_path ()) in
  let hash = trust_hash_identity ~command in
...
let adapter : Harness.t =
  { id = "codex";
    ...
    install =
      (fun ~root ~agent_dir ->
        let result = Harness.install_json_file ~harness_id:"codex" ~path:(hooks_path ()) ~matcher ~root ~agent_dir in
        install_trust ~command:(Harness.guarded_command ~harness_id:"codex" ~root ~agent_dir);
        result);
    uninstall = (fun ~root -> Harness.uninstall_json_file ~harness_id:"codex" ~path:(hooks_path ()) ~root);
    render_feedback = Harness.render_camel
  }

(* ── tests ──────────────────────────────────────────────────────────────────────────────────────*)

let%test "trust hash is stable for a fixed command identity" =
  (* a fixed command string ⇒ a fixed sha256; pins the canonical-identity serialisation *)
  String.length (trust_hash_identity ~command:"python3 -c 'x' # fennec-fastlane:abc") = 7 + 64

let%test "trust insertion appends the hook state table" =
  let table = trust_table ~hooks_path:"/tmp/hooks.json" in
