...
val run :
  ...
  assets:string ->
  unit
(** [?console_bc] is the path of the project's built [console] bytecode; when given ([dev --console]),
    the supervisor spawns it, drives its eval socket, and pins a REPL prompt as the UI's bottom line. *)
