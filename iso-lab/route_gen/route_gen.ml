(* File-tree routing codegen, multi-app. Scans frontend/apps/<name>/ — each app is a
   ...
   files (index.mlx / [id].mlx / [...rest].mlx, nesting freely). Emits ONE module:
     ...
     ...
     let apps : Iso.mount list = [ { base = Shop.base; root = Shop.Layout.make;
...
let () =
  ...
  let mounts = List.map (fun (m, _) ->
    Printf.sprintf "  { Iso.base = %s.base; root = %s.Layout.make; router = %s.router; document = %s.template }"
      m m m m) mods in
  Buffer.add_string buf (Printf.sprintf "\nlet apps : Iso.mount list = [\n%s\n]\n" (String.concat ";\n" mounts));
